...
{{
  ...
  "내용": "FAITHFUL word-for-word translation of EVERY sentence into Korean 개조식. CRITICAL: translate the COMPLETE text — every clause, condition, requirement, species name, document number, and date must appear in Korean. Do NOT omit, merge, compress, or paraphrase any part. 개조식 ending: each sentence ends with a -ㅁ nominalization (됨, 함, 임, 어야 함 etc.). Sentences ending in 됨/함/임/어야 함 are already complete — do NOT add 음. Never ~습니다/~합니다. Keep numbered lists. Use \\n between items. If an input line starts with [B], keep [B] at the start of the corresponding Korean line.",
  ...
  "목적_근거": "Korean 개조식 translation of ONLY the free-text rationale from the 'Objective/rationale text' input field (-ㅁ nominalization endings: 됨/함/임/어야 함; do NOT add 음 to sentences already ending in 됨/함/임). Output empty string if that field is empty or contains only checkboxes. Do NOT include any content from the Description field.",
